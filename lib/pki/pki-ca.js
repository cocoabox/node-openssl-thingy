...
class PkiCA extends PkiChild {
    ...
    async add_client(...conf_args) {
        const days = conf_args[conf_args.length - 1]?.days ?? 9999;
        const bits = conf_args[conf_args.length - 1]?.bits ?? 4096;
        const client = new PkiClient();
        await client.create_rsa_keypair({bits});
        try {
    ...
    async sign_csr_for_child(child , {days} = {}) {
        ...
        const ext = {PkiServer : 'server' , PkiClient : 'client' , PkiCA : 'ca'}[child_pki_member.constructor.name];
    ...
    async revoke(child) {
        if ( ! child ) {
            throw new RangeError('child is empty, expecting PkiChild instance or search parameter e.g. {name:***}');
        }
        ...
        const ext = {PkiServer : 'server' , PkiClient : 'client' , PkiCA : 'ca'}[child.constructor.name];
