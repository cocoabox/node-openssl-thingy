...
class CA extends Child {
    ...
    async add_server(common_name , {
        ...
        if ( ! this.cert ) throw {
            error : 'no-cert' ,
            message : 'This CA has no certs; please call create_cert() first'
        };
        if ( this.is_revoked ) throw {error : 'revoked' , message : 'This CA is revoked; cannot sign anything'};
        ...
        try {
            const serial = await this.sign_csr_for_child(server , {
                alt_names : conf_args?.alt_names ,
                days ,
            });
     ...
     * @param {string|object} common_name CN of the client
     ...
     * @returns {Promise<Server>}
     */
    async add_client(common_name , email , {
                         days = 9999 ,
                         key_args = {} ,
                         conf_args ,
                     } = {}
        ...
        try {
            await client.create_csr(common_name , email , {key_args , conf_args});
        } catch (err) {
     ...
     * @param {string?} conf_args.organization_name
     * @param {string?} conf_args.organization_unit
     * @param {string?} conf_args.crl_url
     * @param {string[]?} conf_args.alt_names
     * @param {string?} crl_url
     * @returns {Promise<Server>}
     */
    async add_ca(common_name , {
        ...
        try {
            const serial = await this.sign_csr_for_child(ca , {days , crl_url});
    ...
    get_child({serial , name , subject} = {}) {
        ...
        if ( typeof arguments[0] === 'string' ) {
            if ( arguments[0].startsWith('/') ) {
                console.warn('[NOTICE] please use .resolve_path() to resolve a path');
                return this.resolve_path(arguments[0]);
            }
            ...
            throw new Error(`expecting .serial or .name or .subject to be provided but given : ${args}`);
        }
    }

    /**
     * resolves a textual, serial-number-based path to the actual Child node
     * @param {string} path an absolute (or relative, though not recommended) path to the current node in the form
     *      /SERIAL_NUMBER_OF_INTERMEDIATE_CA/SERIAL_NUMBER_OF_INTERMEDIATE_CA/.../SERIAL_NUMBER_OF_NODE
     * @returns {Child}
     */
    resolve_path(path) {
        let root = this;
        if ( path.startsWith('/') ) {
            ...
            // don't go to the root node, start from this
        }
        let inspecting = root;
        const serials = path.substring(1).split('/');
        while (serials.length > 0) {
            const serial = serials.shift(); // get the first item
            const children = inspecting?.children ?? {};
            inspecting = children[serial];
            if ( typeof inspecting === 'undefined' ) {
                throw new Error(`could not find serial ${serial} under CA "${inspecting.subject}" ; path was "${path}`);
            }
        }
        return inspecting;
    }
    ...
    async #do_ca_operation(ext , {before , operation , crl_url , alt_names} = {}) {
        ...
        const ca_config = create_conf_data.for_ca_operations(ca_operation_args).conf;
    ...
    async sign_csr_for_child(child , {days , crl_url , alt_names} = {}) {
    ...
    /**
     * revoke one cert; after this, you need to call .get_crl() again to generate a new CRL
     * @param {Child} child a child CA/server/client node
     * @param {string?} reason valid reason :
     *   'unspecified' , 'keyCompromise' , 'CACompromise' , 'affiliationChanged' , 'superseded' ,
     *   'cessationOfOperation' , 'certificateHold' , 'removeFromCRL'
     * @returns {Promise<void>}
     * @see for reason see https://github.com/openssl/openssl/blob/master/apps/ca.c#L2318
     */
    async revoke(child , {reason} = {}) {
        if ( ! child ) {
        ...
        this.crl = null; // we need a new CRL to be generated
        this.emit_update('revoke');

    }

    /**
     * gets validity info etc about the current CRL; if none then a CRL will be generated
    ...
    /**
     * return the Buffer object to the revocation list (CRL) in PEM format;
     * if cache is available it'll be used instead of generating a new one unless force=true
     * @param {boolean?} force
    ...
    /**
     * verify if a given cert is revoked by this CA; if revoked throws {error:..}
     * @param {Buffer|string} cert
     * @param {boolean?} [force_get_crl=false]
     * @param {boolean?} [throw_if_revoked=false]
     * @returns {Promise<{boolean}>}
     *      returns True if cert is OK
     *      returns False if cert is revoked
     *      throws if failed to verify (or if throw_if_revoked is True)
     */
    async verify_revocation_of_cert(cert , {force_get_crl = false , throw_if_revoked = false} = {}) {
        const crl = await this.get_crl({force : force_get_crl});
        const ca_cert_chain = this.cert_chain;
        try {
            await openssl.verify_crl(crl , cert , ca_cert_chain);
            return true;
        } catch ({error , verify_error_reason}) {
            if ( verify_error_reason === 'certificate revoked' ) {
                if ( throw_if_revoked )
                    throw {revoked : true , error , verify_error_reason};
                else return false;
            } else {
                throw {error , verify_error_reason};
            }
        }
    }
}
