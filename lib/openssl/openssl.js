...
async function openssl(action , openssl_args , {
                           ...
                           openssl_location ,
                           stdout_encoding ,
                           reject_on_non_zero_exit_code ,
                           timeout ,
                           show_stderr ,
                       } = {}
) {
    show_stderr = show_stderr ?? false;
    timeout = timeout ?? 2500;
    reject_on_non_zero_exit_code = reject_on_non_zero_exit_code ?? true;
    stdout_encoding = stdout_encoding ?? 'utf8';
    openssl_location = openssl_location ?? 'openssl';
    const temp_files = [];
    const out_files = {};
    ...
    const print_error_func = typeof show_stderr === 'function' ? show_stderr : console.warn;
    const out = {};
    const stdout = [];
    const stderr = [];
    let error_thrown;
    ...
    if ( error_thrown || (out.error && reject_on_non_zero_exit_code) ) {
        console.warn('openssl command failed' +
